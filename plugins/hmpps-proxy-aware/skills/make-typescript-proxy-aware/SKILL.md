...
1. Run validation in the target repo:
   ...
   ```
   A targeted `npm install <package>@<version>` can silently prune unrelated optional/platform packages from the lock file (see Gotchas) — `npm ci` succeeding locally is not sufficient proof; only a clean reinstall reliably surfaces this before CI does.
...
- **A targeted `npm install <package>@<version>` can silently prune unrelated optional/platform-specific packages from `package-lock.json`** (observed with `sass-embedded-*` variants when only changing `https-proxy-agent`'s version), which then fails CI with `npm ci` errors like `Missing: sass-embedded-<platform>@<version> from lock file` even though `npm install` and local tests passed. After any dependency version change, verify with a clean `rm -rf node_modules package-lock.json && npm install` followed by `npm ci` before pushing, and diff the lockfile's `packages` map (added/removed keys, not just line count) to confirm only expected packages moved.
