 ...
 * target repository using `git apply --3way`.
 *
 * Usage: node apply-change.mjs <pr-number> [--dry-run] [--repo <path>]
 *
 * Options:
 *   --dry-run      Show what would be applied without making changes
 *   --repo <path>  Path to the target repository (defaults to current working directory)
 *
...
const repoPath = repoIndex !== -1 ? resolve(args[repoIndex + 1]) : process.cwd()

if (!prNumber) {
  process.stderr.write('Usage: node apply-change.mjs <pr-number> [--dry-run] [--repo <path>]\n')
  process.exit(1)
}
...
try {
  ...
  const skippedFileDetails = prFiles
    .filter(f => skippedFiles.includes(f.filename))
